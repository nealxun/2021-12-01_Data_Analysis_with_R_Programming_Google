...
Your stakeholder tells you that they would like you to share the visualization breaking down payment type by city because it will help inform how the company targets promotions in the future. They ask you to create a cleaned and labeled version and save it as a .png file for them to include in a presentation. 
...
ggplot(data = hotel_bookings) +
  geom_bar(mapping = aes(x = market_segment)) +
  facet_wrap(~hotel) +
  labs(title="Market Segment by Each Hotel Type")
```
...
```{r save your plot}
ggsave('hotel_booking_chart1.png')
```
