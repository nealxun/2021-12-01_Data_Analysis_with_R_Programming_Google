...
```{r load data}
hotel_bookings <- read.csv(glue::glue("{dirname(getwd())}/Input/hotel_bookings.csv"))
```
...
```{r}
ggplot(data = hotel_bookings, mapping = aes(x = stays_in_weekend_nights , y = children)) + 
        geom_point() +
        geom_jitter()
...
df <- hotel_bookings %>% 
        ...
        # group_by(children) %>% 
        # summarise(stays_in_weekend_nights_avg = mean(stays_in_weekend_nights))

ggplot(data = df, mapping = aes(x = children, y = stays_in_weekend_nights)) + 
        geom_boxplot() +
        geom_jitter()
```
...
df <- hotel_bookings %>% 
        mutate(children = as_factor(children)) %>% 
        group_by(children) %>%
        summarise(stays_in_weekend_nights_count = sum(stays_in_weekend_nights > n))

ggplot(data = df, mapping = aes(x = children, y = stays_in_weekend_nights_count)) + 
        geom_col(fill = "steelblue") +
        labs(title = glue::glue("Count of stay in weekend nights > {n}"))
```
